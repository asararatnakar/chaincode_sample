...
CORE_PEER_GOSSIP_IGNORESECURITY=true CORE_PEER_COMMITTER_LEDGER_ORDERER=$ORDERER_IP:7050 peer channel create -c myc1 -a anchorPeer1.txt,anchorPeer2.txt,anchorPeer3.txt >log.txt 2>&1
cat log.txt
echo "===================== channel \"myc1\" is created successfully ===================== "
CORE_PEER_GOSSIP_IGNORESECURITY=true CORE_PEER_COMMITTER_LEDGER_ORDERER=$ORDERER_IP:7050 peer channel create -c myc2 -a anchorPeer3.txt >log.txt 2>&1
cat log.txt
echo "===================== channel \"myc2\" is created successfully ===================== "
echo

for peer in 0 1 2; do
	CORE_PEER_COMMITTER_LEDGER_ORDERER=$ORDERER_IP:7050 CORE_PEER_ADDRESS=peer$peer:7051 peer channel join -b myc1.block >log.txt 2>&1
        cat log.txt
done
echo "===================== All peers joined on the channel \"myc1\" ===================== "

CORE_PEER_COMMITTER_LEDGER_ORDERER=$ORDERER_IP:7050 CORE_PEER_ADDRESS=$PEER2_IP:7051 peer channel join -b myc2.block >log.txt 2>&1
cat log.txt
echo "===================== Peer2 joined on the channel \"myc2\" ===================== "
...
cat log.txt
echo "===================== Query on chaincode on PEER2 \"myc$ch\" ===================== "
done

echo "===================== SubLedger Test execution completed ===================== "
